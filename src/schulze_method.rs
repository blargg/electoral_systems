...
pub fn schulze_method(votes: Vec<Ballot>) -> Vec<Candidate> {
    ...
    let mut candidates_to_sort = count.candidates().collect::<Vec<_>>();
...
pub fn schulze_method_single(votes: Vec<Ballot>) -> Candidate {
    *schulze_method(votes)
        .get(0)
        .expect("Expecting there to be at least one candidate.")
}
...
mod test {
        ...
        length.prop_flat_map(|num_vertecies| {
            (1..num_vertecies).prop_flat_map(move |path_length| {
                square_vec(Just(num_vertecies)).prop_flat_map(move |weights| {
                    shuffled_subsequence(0..num_vertecies, path_length)
                        .prop_map(move |path| (weights.clone(), path))
                })
            })
        })
    }
    ...
    fn simple_floyd_warshall() {
        let edge_weights = vec![vec![0, 5, 0], vec![0, 0, 5], vec![0, 0, 0]];
    ...
    fn simple_floyd_warshall_reversed() {
        let edge_weights = vec![vec![0, 0, 0], vec![5, 0, 0], vec![1, 5, 0]];
        let widest_paths = floyd_warshall_widest_paths(&edge_weights);
        assert_eq!(
            widest_paths,
            vec![
    ...
    fn floyd_warshall_chain() {
        let edge_weights = vec![vec![0, 5, 1], vec![1, 0, 5], vec![3, 1, 0]];
        let widest_paths = floyd_warshall_widest_paths(&edge_weights);
        assert_eq!(
            widest_paths,
            vec![
